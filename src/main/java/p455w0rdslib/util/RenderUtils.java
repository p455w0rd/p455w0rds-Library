...
public class RenderUtils {
	...
	public static void renderEntity(Entity entity, int xPos, int yPos, float scale, float rot) {
		...
		GlStateManager.translate(xPos + 8, (float) yPos + 24, 50F);
		GlStateManager.scale(-scale, scale, scale);
		...
		GlStateManager.translate(xPos, yPos + (entity.height * scale), 50);
		
		GlStateManager.scale((-scale), scale, scale);
		GlStateManager.rotate(180.0F, 0.0F, 0.0F, 1.0F);
		float f1 = entity.rotationYaw;
		float f2 = entity.rotationPitch;
		GlStateManager.rotate(135.0F + rot, 0.0F, 1.0F, 0.0F);
		RenderHelper.enableStandardItemLighting();
		GlStateManager.rotate(-135.0F, 0.0F, 1.0F, 0.0F);
